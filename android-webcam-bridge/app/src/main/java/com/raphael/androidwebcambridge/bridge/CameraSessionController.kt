...
class CameraSessionController(context: Context) {
    ...
    @Volatile
    private var faceDetectCounter = 0
    ...
    fun bind(
        ...
        Camera2Interop.Extender(
            previewBuilder,
        ).setCaptureRequestOption(CaptureRequest.CONTROL_AE_TARGET_FPS_RANGE, Range(settings.frameRate, settings.frameRate))
        Camera2Interop.Extender(
            analysisBuilder,
        ).setCaptureRequestOption(CaptureRequest.CONTROL_AE_TARGET_FPS_RANGE, Range(settings.frameRate, settings.frameRate))
        ...
        val selector =
            if (settings.selectedCameraId != null) {
    ...
    private fun processImageProxy(
        ...
        if (mediaImage != null && activeCamera) {
            // ponytail: throttle face detection to ~12fps, saves battery at 60fps
            if (++faceDetectCounter % 5 == 0) {
                ...
                detector.process(inputImage)
                    ...
                    .addOnFailureListener { Log.w(TAG, "face detection failed: ${it.message}") }
    ...
    private fun finishFrame(
        ...
        try {
            ...
            Log.w(TAG, "finishFrame: ${e.message}")
        } finally {
            imageProxy.close()
        }
    }

    fun applyLiveControls(
        settings: BridgeSettings,
        onStatus: (String) -> Unit = {},
    ) {
        activeSettings = settings
        ...
        if (!settings.focusAuto) {
            // ponytail: CANCEL stops any active AF sweep before switching to OFF — required by some HALs
            ...
            runCatching { activeCamera.cameraControl.cancelFocusAndMetering() }
        ...
        }.onFailure { Log.w(TAG, "setCaptureRequestOptions: ${it.message}") }
    }
    ...
    private fun kelvinToRggbGains(kelvin: Int): FloatArray {
        ...
        val (rT, gT, bT) =
            if (t <= 66f) {
                ...
                Triple(
                    329.698727446 * (t - 60f).pow(-0.1332047592f),
                    288.1221695283 * (t - 60f).pow(-0.0755148492f),
                    255.0,
                )
            }
        val d65r = 255.0
        val d65g = 99.4708025861 * ln(65.0) - 161.1195681661
        val d65b = 138.5177312231 * ln(55.0) - 305.0447927307
        return floatArrayOf(
    ...
    private fun computeFocusPeaking(image: ImageProxy): Bitmap? {
        ...
        for (y in 1 until outH - 1) {
            for (x in 1 until outW - 1) {
                val p5 = ys[y * outW + x]
                val edge =
                    abs(p5 - ys[y * outW + x - 1]) + abs(p5 - ys[y * outW + x + 1]) +
    ...
    private fun ImageProxy.toConstantResolutionJpeg(settings: BridgeSettings): ByteArray {
        ...
        // ponytail: always resize to target resolution for consistent bandwidth
        ...
        if (cw != targetW || ch != targetH) {
            ...
            scaled.compress(Bitmap.CompressFormat.JPEG, settings.jpegQuality.coerceIn(20, 100), out)
    ...
    fun focusAt(
        ...
        try {
            // ponytail: startFocusAndMetering works regardless of AF mode, no need to clobber AE/WB
    ...
    companion object {
        ...
        fun discoverCameras(context: Context): List<LensInfo> {
            ...
            fun build(
                ...
                val maxZoom = chars.get(CameraCharacteristics.SCALER_AVAILABLE_MAX_DIGITAL_ZOOM)?.toFloat() ?: 1.0f
                return LensInfo(
                    cameraId,
                    label,
